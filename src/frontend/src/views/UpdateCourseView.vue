<template>
   ...
   <div class="bg-white p-8 rounded shadow-md w-full max-w-md">
    <div>
      ...
      <form @submit.prevent="updateCourse">
        ...
        <button type="submit" class="bg-indigo-600 text-white py-2 px-4 rounded">Update</button>
      </form>
    </div>
   </div>    
  </div>
  ...
  const fetchCourse = async () => {
    const response = await axios.get(`http://localhost/api/courses/${route.params.id}`, {
      headers: {
  ...
  const updateCourse = async () => {
    await axios.put(`http://localhost/api/courses/${route.params.id}`, form.value, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem('authToken')}`
      }
    });
