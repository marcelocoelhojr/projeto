<template>
    <div>
      <div class="container mx-auto py-10">
        ...
        <form @submit.prevent="handleUpdate" class="bg-white p-6 rounded shadow-md">
          ...
          <button type="submit" class="w-full bg-indigo-600 text-white py-2 px-4 rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">Update</button>
        </form>
      </div>
    </div>
  </template>
  ...
  const handleUpdate = () => {
    axios.put('http://localhost/api/users/profile', user.value, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem('authToken')}`
      }
    })
